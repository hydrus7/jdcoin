...
The build script we'll be using is contained in the JDCOIN github repository ([contrib/gitian-build.py](https://github.com/JdCoinNodes/JDCOIN/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the JDCOIN repository and will need to fetch this script with the following commands:
