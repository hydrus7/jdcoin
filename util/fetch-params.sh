...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/JDCOINParams"
    else
        PARAMS_DIR="$HOME/.jdcoin-params"
    fi
fi
...
Failed to fetch the JDCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
JDCOIN - fetch-params.sh

This script will fetch the JDCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common JDCOIN zkSNARK parameters. Note that it is
