...
bool CWalletDB::WriteZJDCOINeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZJDCOINeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZJDCOINeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZJDCOINeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZJDCOINeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZJDCOINeed_deprecated(uint256& seed)
{
