...
const char * const JDCOIN_CONF_FILENAME = "jdcoin.conf";
const char * const JDCOIN_PID_FILENAME = "jdcoin.pid";
const char * const JDCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
static fs::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JDCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "JDCOINParams";
#else
    // Unix
    return pathRet / ".jdcoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", JDCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", JDCOIN_MASTERNODE_CONF_FILENAME));
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", JDCOIN_PID_FILENAME));
