...
/** Fees smaller than this (in ujdc) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k JDC
    // Mainnet/Testnet block reward reduction schedule
    const int nLast = 100000;
    if (nHeight > nLast)  return 5     * COIN;
    if (nHeight > 90000)  return 6     * COIN;
    if (nHeight > 80000)  return 7     * COIN;
    if (nHeight > 70000)  return 8     * COIN;
    if (nHeight > 60000)  return 9     * COIN;
    if (nHeight > 50000)  return 10    * COIN;
    if (nHeight > 40000)  return 11    * COIN;
    if (nHeight > 30000)  return 12    * COIN;
    if (nHeight > 20000)  return 13    * COIN;
    if (nHeight > 15000)  return 14    * COIN;
    if (nHeight > 10000)  return 15    * COIN;
    if (nHeight > 50000)  return 16    * COIN;
    if (nHeight > 25000)  return 17    * COIN;
    if (nHeight > 1500)   return 18    * COIN;
    if (nHeight > 500)    return 19    * COIN;
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZJDCOINupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zJDC supply", __func__);
        return DISCONNECT_FAILED;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZJDCOINupplyConnect(block, pindex, fJustCheck)) {
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateJDCOINupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
